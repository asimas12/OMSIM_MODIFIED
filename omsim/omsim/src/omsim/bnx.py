...
class BNX:
        ...
        def write_rmap_entry_cut_orig(self, info, rmap, nicks, ofile):
                ...
                for pos in nicks:
                        count += 1
                        rmap += '\t' + '{0:.2f}'.format(pos)
                length = info[1]
                rmap += '\t' + '{0:.2f}'.format(length)
                ofile.write(rmap + '\n')

        def write_rmap_entry_cut_error(self, info, rmap, nicks, ofile, relative_scan_stretch):
                count = 0
                for pos in nicks:
                        count += 1
                        rmap += '\t' + '{0:.2f}'.format(pos * relative_scan_stretch)
                length = info[1] * relative_scan_stretch
                rmap += '\t' + '{0:.2f}'.format(length)
                ofile.write(rmap + '\n')

        def write_rmap_entry_com_error(self, info, rmap, nicks, ofile, relative_scan_stretch):
                count = 0
                last = 0
                for pos in nicks:
                        count += 1
                        if last!=0 or count == len(nicks):
                            rmap += '\t' + '{0:.2f}'.format( abs(last - (pos * relative_scan_stretch) ) )
                        last = pos * relative_scan_stretch
                length = info[1] * relative_scan_stretch
                rmap += '\t' + '{0:.2f}'.format(length)
                ofile.write(rmap + '\n')

        def write_rmap_entry_com_orig(self, info, rmap, nicks, ofile):
                count = 0
                last = 0
                for pos in nicks:
                        count += 1
                        if last!= 0 or count == len(nicks):
                            rmap += '\t' + '{0:.2f}'.format( abs(last - (pos) ) )
                        last = pos
                length = info[1]
                rmap += '\t' + '{0:.2f}'.format(length)
                ofile.write(rmap + '\n')
