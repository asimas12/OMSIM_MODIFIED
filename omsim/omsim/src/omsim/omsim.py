...
def omsim(settings, clean):
        ...
        for chip in range(1, settings.chips + 1):
                ...
                for scan in range(1, settings.scans_per_chip + 1):
                        ...
                        for l, s, e, m, meta, true_rmaps, rf in noise.generate_scan(seq_lens, cum_seq_lens, fns, rns, clean):
                                        moleculeID += 1
                                        molecule = {}
                                        molecule2 = []
                                        lse = [l,s,e]
                                        ...
                                        for label in settings.labels:
                                                if settings.min_nicks <= len(molecule[label]):
                                                        molecules[label].append((l,s,e, molecule[label], chip_settings['scans'], meta, molecule2, rf))
                ...
                for label in settings.labels:
                        ...
                        if clean:
                                ofile2 = open(settings.prefix + '.' + label + '.' + str(chip) + '.original' '.bnx', 'w+')
                                ...
                                ofile3 = open(settings.prefix + '.' + label + '.' + str(chip) + '.with_error' '.cut', 'w+')
                                ofile5 = open(settings.prefix + '.' + label + '.' + str(chip) + '.with_error' '.com', 'w+')
                                ofile7 = open(settings.prefix + '.' + label + '.' + str(chip) + '.with_error' '.lse', 'w+')
                        ...
                        for l, size, e,  m, s, meta, true_rmaps, rf in molecules[label]:
                                moleculeID += 1
                                if clean:
                                    ...
                                    bnx.write_rmap_entry_com_error((moleculeID, l, s), str('_Rmap') + str(count) + ': ', m,
                                                         ofile5, relative_stretch[moleculeID - 1])
                                    bnx.write_rmap_entry_lsh_error((moleculeID, l, s), str('_Rmap') + str(count) + ': ',
                                                                  m,
                                                                  ofile7, relative_stretch[moleculeID - 1], l, size, e, rf)
                                count=count+1
                        if not clean:
                            ofile.close()
                        #bedfile.close()
